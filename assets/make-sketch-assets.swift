// make-sketch-assets.swift — handgekritzelte Assets fuer CallNotes:
//   assets/banner.png        (1280x640, Kritzel-Look)
//   assets/how-it-works.png  (760x1150, Kritzel-Flussdiagramm)
...
func handFont(_ size: CGFloat, bold: Bool = false) -> NSFont {
    for name in [bold ? "BradleyHandITCTT-Bold" : "BradleyHandITCTT-Bold", "Bradley Hand", "MarkerFelt-Wide", "Marker Felt", "ChalkboardSE-Regular", "Chalkboard SE"] {
        if let f = NSFont(name: name, size: size) { return f }
    }
    return NSFont.systemFont(ofSize: size, weight: .medium)
}

// Linie in kleine Segmente zerlegen und jeden Punkt leicht versetzen — der Kritzel-Kern
...
// handgezeichnetes Rechteck: Kanten ragen an den Ecken leicht ueber, zweiter blasser Strich
func sketchRect(_ r: NSRect, color: NSColor, fill: NSColor? = nil, width: CGFloat = 2.6) {
    ...
    let o: CGFloat = 5 // Ecken-Ueberstand
...
func sketchArrow(_ a: NSPoint, _ b: NSPoint, color: NSColor, width: CGFloat = 2.4) {
    // leichte Kurve: Mittelpunkt seitlich versetzen
...
// gekritzelter Telefonhoerer: zwei runde Enden + dicker Bogen + Schallwellen
...
let banner = NSImage(size: NSSize(width: bw, height: bh), flipped: false) { _ in
    ...
    // gekritzelter Notizzettel rechts oben: der "Output" der Geschichte
    ...
    handText("call w/ Anna", center: NSPoint(x: note.midX, y: note.maxY - 28), size: 20, color: violet, rotation: -2)
    ...
    // Pfeil: Telefon -> Zettel (die ganze Story in einem Bogen)
    ...
    handText("calls become notes. automatically.", center: NSPoint(x: 737, y: 305), size: 44, color: violet, rotation: -1)
    handText("on-device transcription · speaker separation · your choice of AI", center: NSPoint(x: 725, y: 245), size: 27, color: ink.withAlphaComponent(0.62), rotation: -0.6)

    // zwei gekritzelte Spuren unten (du + Gegenseite)
    handText("you", center: NSPoint(x: 118, y: 150), size: 26, color: indigo, rotation: -3)
    sketchWaveform(in: NSRect(x: 170, y: 118, width: 1000, height: 60), color: indigo, bars: 46)
    handText("caller", center: NSPoint(x: 112, y: 72), size: 26, color: violet, rotation: -2)
    sketchWaveform(in: NSRect(x: 170, y: 42, width: 1000, height: 56), color: violet, bars: 46)
    return true
}
savePNG(banner, to: assetsDir.appendingPathComponent("banner.png"), w: 1280, h: 640)
print("OK: assets/banner.png (Kritzel)")

// ---------- How it works (760x1150, gekritzeltes Flussdiagramm) ----------
let dw: CGFloat = 760, dh: CGFloat = 1150
let diagram = NSImage(size: NSSize(width: dw, height: dh), flipped: false) { _ in
    bgDark.setFill()
    ...
    let b1 = box(cx, 1080, 400, 84, "call detected", "FaceTime · WhatsApp · Zoom · Teams", indigo, rot: -0.8)

    // Gabel auf zwei Spuren
    let b2a = box(cx - 172, 940, 300, 80, "your mic", "own track", indigo, rot: -1)
    let b2b = box(cx + 172, 940, 300, 80, "caller audio", "process tap — no drivers", violet, rot: 0.8)
    ...
    handText("2 separate tracks!", center: NSPoint(x: dw - 96, y: 1013), size: 20, color: green, rotation: -7)

    let b3 = box(cx, 800, 430, 84, "whisper transcription", "on-device (or Groq API)", violet, rot: -0.5)
    ...
    let b4 = box(cx, 662, 430, 84, "speaker separation", "sherpa-onnx · 100% local", violet, rot: 0.6)
    sketchArrow(NSPoint(x: b3.midX, y: b3.minY - 4), NSPoint(x: b4.midX, y: b4.maxY + 6), color: ink)
    handText("who said what?", center: NSPoint(x: 92, y: 731), size: 20, color: ink.withAlphaComponent(0.5), rotation: 6)

    let b5 = box(cx, 524, 430, 84, "AI summary", "Claude · any OpenAI API · Ollama · none", indigo, rot: -0.7)
    sketchArrow(NSPoint(x: b4.midX, y: b4.minY - 4), NSPoint(x: b5.midX, y: b5.maxY + 6), color: ink)
    handText("your choice!", center: NSPoint(x: dw - 92, y: 593), size: 20, color: green, rotation: -6)

    let b6 = box(cx, 376, 450, 96, "markdown note", "summary · to-dos · full transcript", green, rot: 0.5)
    ...
    // Ablage-Faecher unten
    let targets = ["Obsidian", "Apple Notes", "Nextcloud", "Notion", "external drive"]
    let tw: CGFloat = 132
    let total = tw * CGFloat(targets.count) + 12 * CGFloat(targets.count - 1)
    ...
    for t in targets {
        ...
        handText(t, center: NSPoint(x: tx, y: 227), size: 18.5, color: ink.withAlphaComponent(0.85), rotation: rnd(-2, 2))
    ...
    handText("~1 minute after you hang up. no cloud required.", center: NSPoint(x: cx, y: 128), size: 26, color: violet, rotation: -1.2)
    sketchLine(NSPoint(x: cx - 250, y: 100), NSPoint(x: cx + 250, y: 94), color: violet.withAlphaComponent(0.5), width: 3, amp: 3.5)
    return true
}
savePNG(diagram, to: assetsDir.appendingPathComponent("how-it-works.png"), w: 760, h: 1150)
print("OK: assets/how-it-works.png (Kritzel)")
